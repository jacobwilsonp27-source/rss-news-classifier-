...
async def lifespan(app: FastAPI):
    logger.info("API Started - Politics News Intelligence Module")
    create_table()
    yield
 
 
app = FastAPI(
    title       = "Politics News Intelligence API",
    description = "AI-Powered News Categorization Platform — Politics Module",
    version     = "1.0.0",
    lifespan    = lifespan,
)
...
@app.post("/fetch", summary="Fetch and process latest politics news")
def fetch_and_process():
    """
    Triggers the full pipeline:
    RSS Fetch → Categorize → Sentiment → Store in DB
...
# ── GET /news ─────────────────────────────────────────────────────────────────
@app.get("/news", summary="Get all Politics news articles")
def get_news():
    """
    Returns only Politics category articles from the database.
    """
    articles = get_articles_by_category("Politics")
...
def get_news_by_category(category: str):
    """
    This is the Politics module.
    Only returns articles for 'Politics' category.
    ...
    articles = get_articles_by_category("Politics")
    return {"category": "Politics", "total": len(articles), "articles": articles}
